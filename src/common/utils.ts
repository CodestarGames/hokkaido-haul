...
export class LinkedQueue<T, K = any> {
    ...
    addToFront(k: any, obj?: any): void {
        ...
        if (this.lookup.get(k)) {
            throw new Error(`The following object/value already exists in the queue. ${this.lookup.get(k).key}` +
...
export function fadeBetweenScenes (game: any, sceneKeyFrom: string, sceneKeyTo: string) {
    ...
    let sceneTo = game.scene.getScene(sceneKeyTo);
    if(!sceneTo) {
    ...
    sceneFrom.cameras.main.fade(100, 131, 169, 126, false, (cam, done) => {
        ...
        if(done === 1) {
            ...
            controllerScene.time.delayedCall(250, () => {

                let parentZonePos = {x: 0, y: 0};
                ...
                gameWindow.cameras.main.fadeIn(200, 131, 169, 126);
                //gameWindow.cameras.main.setZoom(.25);

                //game.scene.swapPosition(sceneKeyFrom, sceneKeyTo);
                //controllerScene.gameWindow.scene.stop(sceneKeyFrom);
